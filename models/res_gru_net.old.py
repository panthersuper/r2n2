...
from lib.layers import TensorProductLayer, ConvLayer, PoolLayer, Unpool3DLayer, \
    ...
    EltwiseMultiplyLayer, get_trainable_params, PrintShape, isBoundary
...
class ResidualGRUNet(Net):
    ...
    def network_definition(self):
        ...
        n_fc_filters = [1024]
        n_deconvfilter = [128, 128, 128, 64, 32, 3] #3 classes
        ...
        s_all = s_update[0]
        s_last = s_all[-1]
        gru_s = InputLayer(s_shape, s_last)  # batch, 4,  128, 4, 4

        # change to 3 chaneals
        conv4by4 = Conv3DLayer(gru_s, (3, 3, 3, 3))
        # conv4by4 = theano.gradient.zero_grad(conv4by4.output) 
        result4by4 = isBoundary(conv4by4.output) # level 1, 4 by 4

        conv4by4_del = Conv3DLayer(conv4by4, (n_deconvfilter[0], 3, 3, 3))
        gru_s = AddLayer(gru_s, conv4by4_del)

        #-------------------------------------------
        unpool7 = Unpool3DLayer(gru_s)  # batch, 8,  128, 8, 8
        ...
        unpool8 = Unpool3DLayer(res7) # batch, 16,  128, 16, 16
        ...
        unpool9 = Unpool3DLayer(res8) # batch, 32,  64, 32, 32
        ...
        conv10a = Conv3DLayer(res9, (n_deconvfilter[4], 3, 3, 3)) # batch, 32,  32, 32, 32 
        ...
        conv11 = Conv3DLayer(res10, (n_deconvfilter[5], 3, 3, 3))  # batch, 32,  3, 32, 32 
        softmax_loss = SoftmaxWithLoss3D(conv11.output) 

        PrintShape(conv11.output)
        ...
        self.boundarytest = result4by4.prediction()

        # print("-----------------------------------------------------")
        # print(self.x,"input")
        # print(conv11.output,"output")
        # print("-----------------------------------------------------")
